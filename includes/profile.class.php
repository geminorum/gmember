...
class gMemberProfile extends gPluginModuleCore
{
	...
	public function manage_users_custom_column( $empty, $column_name, $user_id )
	{
		...
		$lastactive  = function_exists( 'bp_get_last_activity' ) ? bp_get_last_activity( $user_id ) : FALSE;

		$html .= '<table w1idth="100%;"></tbody>';
		// $html .= mysql2date( ( 'list' == $mode ? 'Y/m/d' : 'Y/m/d \<\b\r \/\> g:i:s a' ), $user->user_registered );
		$html .= '<tr><td>'.__( 'Registered', GMEMBER_TEXTDOMAIN ).'</td><td><code title="'.mysql2date( 'g:i:s a', $user->user_registered ).'">'.mysql2date( 'Y/m/d', $user->user_registered ).'</code></td></tr>';
		$html .= '<tr><td>'.__( 'Register IP', GMEMBER_TEXTDOMAIN ).'</td><td><code>'.( $register_ip ? $register_ip : __( 'N/A', GMEMBER_TEXTDOMAIN ) ).'</code></td></tr>';
		$html .= '<tr><td>'.__( 'Last Login', GMEMBER_TEXTDOMAIN ).'</td><td>'.( $lastlogin ? '<code title="'.mysql2date( 'g:i:s a', $lastlogin ).'">'.mysql2date( 'Y/m/d', $lastlogin ) : __( 'Never', GMEMBER_TEXTDOMAIN ) ).'</code></td></tr>';

		if ( $lastactive ) // https://wordpress.org/plugins/buddypress-last-active-users-wp-admin/
			$html .= '<tr><td>'.__( 'Last Active', GMEMBER_TEXTDOMAIN ).'</td><td>'.( $lastactive ? $lastactive : __( 'Never', GMEMBER_TEXTDOMAIN ) ).'</code></td></tr>';
